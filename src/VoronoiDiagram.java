...
public class VoronoiDiagram extends JPanel implements MouseListener {
    ...
    /**
     * Goes through one pixel at a time comparing it to each dot(Pixel) currently on the screen
...
//        }
//    }
    private void findShapesThread() {
//        ExecutorService executorService = Executors.newFixedThreadPool(numThreads);
//        long startTime = System.nanoTime();
        System.out.println(this.list.get(0).getImportantPoints());
        for (int x = 0; x < 4; x ++) {
            for (int y = 0; y < 4; y ++) {
...
//        System.out.println("Dur FindShape:  " + dur);
    ...
    private BufferedImage getColoredImage() {
        ...
        long endTime = System.nanoTime();
        long dur = (endTime - startTime) / 1000000;
        System.out.println("Dur getImage:  " + dur);
        return bi;
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        long endTime = System.nanoTime();
        long dur = (endTime - startTime) / 1000000;
        System.out.println("Dur PaintComonent:  " + dur);
    }
    ...
    public void mouseClicked(MouseEvent e) {
        Point pnt = new Point(e.getX(), e.getY());
            list.add(pnt);
            if (list.size() > 0) findShapesThread();
    }
